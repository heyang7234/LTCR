...
class COMET:
    ...
    def __init__(
        ...
        self.fc3.to(device)
        # trend 2
        self.fc11 = nn.Linear(2*self.output_dims, self.output_dims)
        self.fc21 = nn.Linear(self.output_dims, self.output_dims)
        self.fc31 = nn.Linear(self.output_dims, 4) # 128*3 + 42 = 426 ，
        self.fc11.to(device)
        self.fc21.to(device)
        self.fc31.to(device)
                    ...
                    if selected_tids.shape[0] > 1:
                        ...
                        for selected_tid in selected_tids:
                            ...
                            for i in range(4):
                                ...
                                cycle640_cls.append(tt.view(1,2)) #     
                                cycle640_cls_labels.append(mapping_dict1[-1])
                            cycle640_contrast[0].append(selected_samples[0].unsqueeze(0).permute(0, 2, 1)) # 
                            cycle640_contrast[0].append(selected_samples[1].unsqueeze(0).permute(0, 2, 1)) # 
                            cycle640_contrast[0].append(selected_samples[2].unsqueeze(0).permute(0, 2, 1)) # 
                            cycle640_contrast[0].append(selected_samples[3].unsqueeze(0).permute(0, 2, 1)) #
                            cycle640_contrast[1].append(selected_samples[4].unsqueeze(0).permute(0, 2, 1)) # 
                            cycle640_contrast[1].append(selected_samples[5].unsqueeze(0).permute(0, 2, 1)) # 
