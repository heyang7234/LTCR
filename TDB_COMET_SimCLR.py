...
import copy
import sklearn
from ts2vec_utils.losses import hierarchical_contrastive_loss
from ts2vec_utils.utils import take_per_row, split_with_nan, centerize_vary_length_series, torch_pad_nan
import math
from simclr_utils.simclr_loss  import simclr_loss
from simclr_utils.augmentations import time_flip_transform_vectorized
...
class COMET:
    ...
    def __init__(
        ...
        self.projection = SimCLRHead(input_size=self.output_dims)
    ...
    def fit(self, X, y, shuffle_function='trial', masks=None, factors=None, n_epochs=None, n_iters=None, verbose=True):
        ...
        mlp_parameters.extend(list(self.fc11.parameters()))
        mlp_parameters.extend(list(self.fc21.parameters()))
        mlp_parameters.extend(list(self.fc31.parameters()))
        projection_parameters = list(self.projection.parameters())
        # 
        model_parameters = list(self._net.parameters())
        all_parameters = model_parameters + mlp_parameters + projection_parameters
        ...
        while True:
            ...
            for x, y in train_loader:
                ...
                out1 = self.projection(self._net(-1.0*x)) # 
                # tx = np.array(x.cpu())
                # tx = rotation_transform_vectorized(tx)
                # txx = torch.from_numpy(tx).to(self.device)
                # out2 = self.projection(self._net(txx)) # rotation    
                out2 = self.projection(self._net(torch.flip(x, dims=[1])))  
                         
                # out1 = self.projection(self._net(x))
                # out2 = self.projection(self._net(x))
                loss = simclr_loss(out1,out2)
...
class SimCLRHead(nn.Module): # 
    def __init__(self, input_size, hidden_1=256, hidden_2=128, hidden_3=50):
        super(SimCLRHead, self).__init__()

        self.projection_1 = nn.Linear(input_size, hidden_1)
        self.projection_2 = nn.Linear(hidden_1, hidden_2)
        self.projection_3 = nn.Linear(hidden_2, hidden_3)

    def forward(self, x):
        x = F.relu(self.projection_1(x))
        x = F.relu(self.projection_2(x))
