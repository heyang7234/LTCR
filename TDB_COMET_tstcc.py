...
import copy
import sklearn
from tstcc_utils.augmentations import DataTransform
from tstcc_utils.TC import TC
from tstcc_utils.loss import NTXentLoss
...
class COMET:
    ...
    def fit(self, X, y, shuffle_function='trial', masks=None, factors=None, n_epochs=None, n_iters=None, verbose=True):
        ...
        all_parameters = model_parameters + mlp_parameters + list(self.temporal_contr_model.parameters())
        ...
        while True:
            ...
            for x, y in train_loader:
                ...
                # print(x.cpu().permute(0, 2, 1).shape)
                aug1, aug2 = DataTransform(np.array(x.cpu().permute(0, 2, 1))) # make sure the Channels in second dim
                aug1, aug2 = torch.from_numpy(aug1).permute(0, 2, 1).float().to(self.device), torch.from_numpy(aug2).permute(0, 2, 1).float().to(self.device)
                features1 = self._net(aug1).permute(0, 2, 1)
                features2 = self._net(aug2).permute(0, 2, 1)
                ...
                lambda1 = 1
                lambda2 = 0.7
                nt_xent_criterion = NTXentLoss(self.device, 256, 0.2,
                                            True)
                
                loss = (temp_cont_loss1 + temp_cont_loss2) * lambda1 

                if(zis.shape[0] == 256):
                    loss = loss +  nt_xent_criterion(zis, zjs) * lambda2
                    ...
                    if selected_tids.shape[0] > 1:
                        ...
                        for selected_tid in selected_tids:
                            ...
                            for i in range(4):
                                ...
                                tt = (self.fc1(torch.cat([self.gap(selected_samples[i]),self.gap(selected_samples[i+5])], dim=0).squeeze()))
                                tt = (self.fc2(tt))
                                tt = self.fc3(tt)
                                cycle640_cls.append(tt.view(1,2)) 
                                cycle640_cls_labels.append(mapping_dict1[1])
                                tt = (self.fc1(torch.cat([self.gap(selected_samples[i+5]),self.gap(selected_samples[i])], dim=0).squeeze()))
                                tt = (self.fc2(tt))
                                tt = self.fc3(tt)
                                cycle640_cls.append(tt.view(1,2)) 
                                cycle640_cls_labels.append(mapping_dict1[-1])
                            ...
                            cycle640_contrast[1].append(selected_samples[4].unsqueeze(0).permute(0, 2, 1)) # (1,T,B)
                            cycle640_contrast[1].append(selected_samples[5].unsqueeze(0).permute(0, 2, 1)) # (1,T,B)
                            cycle640_contrast[1].append(selected_samples[6].unsqueeze(0).permute(0, 2, 1)) # (1,T,B)
                            cycle640_contrast[1].append(selected_samples[7].unsqueeze(0).permute(0, 2, 1)) # (1,T,B)
