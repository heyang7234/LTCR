...
#      
def my_trial_contrastive(z1, z2, z3,  loss_func, hierarchical=False, factor=1.0):
...
def rand_contrastive_loss(z1, z2 ,z3): 

    B, T = z1.size(0), z1.size(1)
    if B == 1:
        return z1.new_tensor(0.)
    z = torch.cat([z1, z2, z3], dim=0)  # (3B) x O x C 
    z = z.transpose(0, 1)  # O x (3B) x C
    sim = torch.matmul(z, z.transpose(1, 2))  # O x (3B) x (3B)

    logits = torch.tril(sim, diagonal=-1)[:, :, :-1]  # O x (3B) x (3B-1), left-down side, remove last zero column
    logits += torch.triu(sim, diagonal=1)[:, :, 1:]  # O x (3B) x (3B-1), right-up side, remove first zero column
